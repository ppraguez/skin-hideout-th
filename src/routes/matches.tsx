...
function Matches() {
  ...
  return (
    <AppLayout>
      <h1 className="font-display text-3xl sm:text-4xl font-bold">Match Hub</h1>
      <p className="text-sm text-muted-foreground mt-1">All times shown in Bangkok (GMT+7).</p>

      <div className="mt-8 flex gap-2">
        {(["live", "upcoming", "done"] as const).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`px-4 py-2 rounded-lg text-sm font-medium border transition capitalize ${
              tab === t ? "border-primary/60 bg-primary/10 text-primary" : "border-border text-muted-foreground hover:text-foreground"
            }`}
          >
            {t === "live" && "🔴 "}{t === "done" ? "Results" : t}
          </button>
        ))}
      </div>
      ...
      <div className="mt-6 grid md:grid-cols-2 xl:grid-cols-3 gap-4">
        {filtered.length === 0 ? (
          <div className="glass-card rounded-2xl p-10 text-center md:col-span-2 xl:col-span-3">
            ...
            <p className="text-sm text-muted-foreground">No matches in this view yet — check back soon.</p>
          </div>
        ) : filtered.map((m) => (
          <div key={m.id} className="glass-card rounded-2xl p-6 hover:glow-border transition">
            ...
            <div className="flex items-center justify-between mt-5 pt-4 border-t border-border text-xs">
              ...
              {m.status === "live"
                ...
                : <button className="text-primary hover:underline">View match →</button>}
            </div>
          </div>
        ))}
      </div>
